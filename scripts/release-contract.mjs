...
export function readTargetMatrix() {
  ...
  if (!Array.isArray(matrix) || matrix.length !== 4) throw new Error("release matrix must contain exactly four targets");
  let previous = "";
  const seen = new Set();
  for (const [index, entry] of matrix.entries()) {
    ...
    if (!/^(?:aarch64|x86_64)-(?:apple-darwin|unknown-linux-gnu)$/.test(entry.target)) {
